...
FORCE_JOIN_CHANNEL = "MythicVoice"  # Don't add @
...
async def check_force_join(client, message):
    try:
        ...
        if user.status not in ["member", "administrator", "creator"]:
            invite_link = await client.create_chat_invite_link(FORCE_JOIN_CHANNEL)
            await message.reply_text(
                "🚫 நீங்கள் முதலில் எங்கள் சேனலில் சேர வேண்டும்:\n\n📢 https://t.me/" + FORCE_JOIN_CHANNEL,
                reply_markup=InlineKeyboardMarkup([
                    [InlineKeyboardButton("✅ சேனலில் சேர", url=invite_link.invite_link)],
...
async def admin_panel(_, message: Message):
    await message.reply_text(
        "**👑 நிர்வாக பக்கம்:**\n\n"
        f"- Force Join சேனல்: @{FORCE_JOIN_CHANNEL}\n"
        f"- உங்கள் ஐடி: `{ADMIN_ID}`",
        reply_markup=InlineKeyboardMarkup([
    ...
    try:
        ...
        if update.old_chat_member.status != update.new_chat_member.status:
            if update.new_chat_member.status == "member":
                await client.send_message(ADMIN_ID, f"✅ குழுவில் சேர்ந்தார்: [{user.first_name}](tg://user?id={user.id})\n📛 {update.chat.title}")
            elif update.new_chat_member.status == "left":
                await client.send_message(ADMIN_ID, f"❌ விட்டு சென்றார்: [{user.first_name}](tg://user?id={user.id})\n📛 {update.chat.title}")
    except Exception as e:
...
async def handle_channel_membership_change(client: Client, update: ChatMemberUpdated):
    try:
        ...
        if update.old_chat_member.status != update.new_chat_member.status:
            if update.new_chat_member.status == "member":
                await client.send_message(ADMIN_ID, f"📡 சேனலில் சேர்ந்தார்: [{user.first_name}](tg://user?id={user.id})\n📛 {update.chat.title}")
            elif update.new_chat_member.status == "left":
                await client.send_message(ADMIN_ID, f"📤 விட்டு சென்றார்: [{user.first_name}](tg://user?id={user.id})\n📛 {update.chat.title}")
    except Exception as e:
